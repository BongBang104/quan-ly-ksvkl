        ...
        <View style={styles.modalContainer}>
          <View style={styles.header}>
            <View style={{flexDirection: 'row', alignItems: 'center', gap: 8}}>
              {step === 2 && <TouchableOpacity onPress={() => setStep(1)}><Feather name="arrow-left" size={20} color="#fff"/></TouchableOpacity>}
              <Text style={styles.headerTitle}>{step === 1 ? 'Chọn Nguồn Nhân Sự' : `Chọn từ ${source === 'ON_CALL_LIST' ? 'ON-CALL' : source}`}</Text>
            </View>
            <TouchableOpacity onPress={resetAndClose}><Feather name="x" size={20} color="#fff" /></TouchableOpacity>
          </View>
          
          <View style={styles.subHeader}>
            <Text style={styles.subHeaderText}>Đang thêm vào: <Text style={{fontWeight: 'bold'}}>Ca {shiftCode} - {label}</Text></Text>
          </View>
          ...
          <View style={styles.body}>
            {step === 1 ? (
              <View style={styles.grid}>
                {roleType !== 'RESERVE' && (
                    <TouchableOpacity style={styles.btnOnCall} onPress={() => handleSelectSource('ON_CALL_LIST')}>
                      <Feather name="phone-call" size={16} color="#c2410c" />
                      <Text style={styles.btnOnCallText}>Điều động từ danh sách ON-CALL</Text>
                    </TouchableOpacity>
                )}
                <View style={styles.teamGrid}>
                  {settings.teams.map(t => (
                    <TouchableOpacity key={t} style={styles.btnTeam} onPress={() => handleSelectSource(t)}><Text style={styles.btnTeamText}>{t}</Text></TouchableOpacity>
                  ))}
                </View>
                <TouchableOpacity style={styles.btnCustom} onPress={() => handleSelectSource('CUSTOM')}>
                  <Feather name="edit-3" size={16} color="#7e22ce" />
                  <Text style={styles.btnCustomText}>Nhập tên thủ công (Người ngoài)</Text>
                </TouchableOpacity>
              </View>
            ) : (
              source === 'CUSTOM' ? (
                <View>
                  <TextInput style={styles.input} autoFocus placeholder="Nhập họ tên KSVKL..." value={customName} onChangeText={setCustomName} />
                  <TouchableOpacity style={styles.btnSave} onPress={() => handleConfirm(customName)}><Text style={styles.btnSaveText}>Xác nhận chèn vào lịch</Text></TouchableOpacity>
                </View>
              ) : (
                <ScrollView style={styles.listArea}>
                  {(source === 'ON_CALL_LIST' ? getOnCallEmployees() : employees.filter(e => e.team === source)).map((e, idx) => (
                    <TouchableOpacity key={e.uId || e.id || idx} style={styles.listItem} onPress={() => handleConfirm(e)}>
                      <Text style={styles.listItemText}>
                          {e.name} <Text style={{color: '#64748b', fontSize: 12, fontWeight: 'normal'}}>
                  ...
                  {(source === 'ON_CALL_LIST' ? getOnCallEmployees() : employees.filter(e => e.team === source)).length === 0 && (
                    <Text style={styles.emptyText}>Không có nhân sự nào trong danh sách này.</Text>
                  )}
                </ScrollView>
              )
            )}
...
const styles = StyleSheet.create({
  ...
  modalContainer: { backgroundColor: '#fff', borderRadius: 12, width: '100%', maxWidth: 350, overflow: 'hidden', elevation: 5 },
