...
class ComplianceChecker:
    ...
    def _check_position_recency(self, shifts):
        ...
        if window is None:
            return out
        # gom (ngày, shift_id) cho từng vị trí, từ tất cả các phiên
...
if __name__ == "__main__":
    ...
    d1_6 = datetime(2026, 6, 1, 6, 0)
    d2_6 = datetime(2026, 6, 2, 6, 0)

    shifts = [
        # 101 (full): một ca, luân phiên CTL -> APP, đều hợp lệ
        Shift(1, 101, "Nguyễn Văn A (full)", d1_6, d1_6 + timedelta(hours=8), sessions=[
            sess(Position.CTL, d1_6, 4),
            sess(Position.APP, d1_6 + timedelta(hours=4), 4),
        ]),
        # 102 (TWR/GCU): luân phiên TWR (ok) -> APP (VI PHẠM phủ năng định);
        # đồng thời phiên TWR dài 5 giờ = 300 phút > 120 -> cảnh báo ngồi vị trí quá lâu
        Shift(2, 102, "Trần Thị B (TWR/GCU)", d2_6, d2_6 + timedelta(hours=8), sessions=[
            sess(Position.TWR, d2_6, 5),
            sess(Position.APP, d2_6 + timedelta(hours=5), 3),
        ]),
    ]

    print(format_report(ComplianceChecker(cfg).check_all(shifts, qualifications=quals)))
